...
import { useTvlVolumeUsdQuery } from "@/graphql/generated/graphql";
...
const IconCard: React.FC<React.PropsWithChildren<IconCardProps>> = ({
  ...
  return (
    <div
      className="relative w-full  border-[] bg-[#D9D9D9] rounded-3xl"
...
> = ({ headingText, bodyText, titleColor, valueColor }) => {
  ...
  return (
    ...
    //     </div>
    //   ) : (
    //     /* <Heading color="#151419" mb="24px" scale="lg">{remainingWords}</Heading>*/
    //     <div
    //       className={`text-[${valueColor}] font-mitr font-bold lg:mb-10 mb-4 text-3xl`}
    //     >
    //       {remainingWords}
...
const Stats = () => {
  ...
  const formattedTvl = data?.factories[0].totalValueLockedUSD
    ? new Intl.NumberFormat("en-US", {
      ...
      }).format(parseFloat(data.factories[0].totalValueLockedUSD))
    : "SOON";

  const formattedVolume = data?.factories[0].totalVolumeUSD
    ? new Intl.NumberFormat("en-US", {
        style: "currency",
        currency: "USD",
        notation: "compact",
        maximumFractionDigits: 2,
      }).format(parseFloat(data.factories[0].totalVolumeUSD))
    : "SOON";
  ...
  return (
    <div className="mb-[10rem]">
      <div className="items-center mb-4">
        ...
        <span className="text-[] lg:text-[50px] lg:leading-[55px] text-[30px] leading-[35px] font-mitr font-bold">
          Bitcoin
        </span>
      </div>
      <div className="text-white font-mitr font-bold lg:text-[65px] lg:leading-[55px] text-[25px] mb-8"></div>
      ...
      <div className="lg:hidden block mb-14">
        <div className="text-white font-mitr mb-8  lg:text-[16px] lg:leading-[22px] text-[20px] leading-[30px]">
          Monark is your all-in-one solution for trading, earning and launching
          projects on Rootstock.
        </div>
      </div>
      <div className="flex flex-col lg:flex-row max-w-full gap-6">
        ...
        >
          <StatCardContent
            ...
            headingText={loading ? "Loading..." : `${formattedTvl}`}
        ...
        <IconCard {...VolumeTradeData}>
          <StatCardContent
            ...
            headingText={loading ? "Loading..." : `${formattedVolume}`}
